...
namespace KDE
{
    ...
    public partial class frmRptDailyPaymentPartyViewer : Form
    {
        frmRptItemWise formlot = new frmRptItemWise();

        public frmRptDailyPaymentPartyViewer()
        {
            InitializeComponent();
        ...
        private void ReportViewer_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                string str;
                con.Open();
                str = "select CONVERT(date, DATE, 106) AS date, Party_Name, rs  from Payment_Details where Date between '" + DateTime.Parse(formlot.Txt_From_Date.Text).ToString("MM/dd/yyyy") + "' and '" + DateTime.Parse(formlot.Txt_To_Date.Text).ToString("MM/dd/yyyy") + "' ";
                ...
                report = @"Report\\DailyPartyWisePaymentDetails.rpt";
                rpt.Load(report);
